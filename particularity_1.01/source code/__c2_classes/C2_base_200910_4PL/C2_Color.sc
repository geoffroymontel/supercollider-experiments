/*
// the following will being up a colour picking gui
// clicking on a colour will give you the code in the post window
// which can be copy-pasted
(
C2_Color.test;
)
*/
...
C2_Color {
	...
	*initClass {
		...
		26.do { |i|
			guide = guide ++ [i.asDigits(3, 3)];
		};
		guide = guide.copyToEnd(1);
		
		rgb = [ [], [], [] ];
		
		guide.do { |item, index|
		
			guide[index].do { |item2, index2|
			
				case
					{item2 == 0}
					{rgb[index2] = rgb[index2] ++ Array.series(100, 0, 0)}
					{item2 == 1}
					{rgb[index2] = rgb[index2] ++ Array.series(100, 0, 0.01)}
					{item2 == 2}
					{rgb[index2] = rgb[index2] ++ Array.series(100, 1.0, 0)};
	...
	*newFromFloat { |float = 0.0|
	
		// value from 0.0 to 1.0
		
		var pos;
		
		pos = [0, (rgb[0].size - 1)].asSpec.map(float.clip(0.0, 1.0));
		pos = pos.asInt;
		^Color.fromArray([rgb[0][pos], rgb[1][pos], rgb[2][pos]]);
		
	}
	
	*test {
	
		var w, cv, sc, a, col, k, d, rect, states;
		var x, y;
		var boxesPerColumn = 50, boxCount = 0, xShift = 0;
		var width = 40;
		
		w = SCWindow.new("c2 color test", Rect(300, 10, 600, 800));
		w.front;
		cv = SCCompositeView(w, Rect(0, 0, 600, 800));
		
		d = dict;

		a = d.asArray;
		k = d.keys.asArray.sort;
		states = Array.newClear(a.size);
		sc = Array.newClear(a.size);
		for(0, (a.size - 1),
			{
			arg c;
			a.put(c, d.at(k.at(c)));
			states.put(c, [k.at(c).asString, a.at(c), a.at(c)]);
			
			y = (boxCount * 14) + 10;
			x = xShift * width;
			rect = Rect(x, y, width, 14);
			
			sc.put(c, C2_ScrollTextBox(cv, rect, slowness:2));
			sc.at(c).behaviour_(\button);
			sc.at(c).font_(Font("Helvetica", 10));
			sc.at(c).states_([states.at(c)]);
			sc.at(c).action_(
				{
				("[C2_Color('" ++ sc.at(c).states.at(0).at(0) ++ "')]").postln;
				}		
			);
			
			boxCount = boxCount + 1;
			if(boxCount > boxesPerColumn,
				{
				boxCount = 0; xShift = xShift + 1
				}
			);
